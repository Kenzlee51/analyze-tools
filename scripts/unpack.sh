...
# =============================================================================
# ПРЕДВАРИТЕЛЬНАЯ НОРМАЛИЗАЦИЯ ИМЁН В src/
# =============================================================================
# Этот проход ОБЯЗАН идти до того, как имена проектных папок попадут в
# basename() и в путь "$UNPACKED_DIR/<project_name>".
#
# Почему именно здесь: normalize.py обходит содержимое каталога, переданного
# в --content-dir, через os.walk и чинит имена ДОЧЕРНИХ файлов и папок. Сам
# каталог-корень он переименовать не может — это точка отсчёта обхода, а не
# его результат. Per-project вызов ниже получает уже битый путь
# unpacked/<битое_имя> в качестве корня, поэтому имя самой проектной папки
# там никогда не чинилось (в логе это видно как "Починено имён: 0"), и
# битое имя из src/ дословно копировалось в unpacked/.
#
# Здесь же корнем выступает $PROJECTS_DIR, а проектные папки являются его
# обычными дочерними элементами — и успешно переименовываются. Проход идёт
# в один поток до параллельного цикла, поэтому гонок за переименование нет.
#
# Примечание: normalize.py идемпотентен (корректные имена пропускаются через
# is_broken()), поэтому повторные запуски безопасны.
normalize_projects_dir() {
    if [[ ! -f "$NORMALIZE_PY" ]]; then
        echo "[WARN] normalize.py не найден: $NORMALIZE_PY"
        echo "[WARN] Пропускаю предварительную нормализацию имён в src/."
        echo "[WARN] Проекты с некорректной кодировкой имён сохранят битые имена в unpacked/."
        return 0
    fi

    local pre_log="$LOG_DIR/normalize_src.log"
    local t0 rc=0
    t0=$(now_ns)
